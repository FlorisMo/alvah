...
## Fase 11 — Tone-of-voice review — KLAAR

**Status**: afgerond in april 2026. Alle vijf sub-fases (script + baseline,
mechanische sweeps, pagina-per-pagina, content collections, cross-cutting +
borging) zijn doorlopen.

**Resultaat**:
- 525 em-streepjes → 0
- Alle 11 pagina's, 5 milestones, 19 vragen, 40 oefeningen door de
  ToV-checklist gehaald
- 11 hits bewust behouden (boektitels, counter-example-quotes, verbatim-
  citaten) — gedocumenteerd in `docs/tov-check.md`
- `scripts/check-tov.mjs` draait nu bij elke commit via `.githooks/pre-commit`
  en blokkeert regressies in em-streepje, gender en retorische-tag

**Infrastructuur die overblijft**:
- `scripts/check-tov.mjs` — de ToV-scanner (strict en non-strict modes)
- `.githooks/pre-commit` — automatisch op commit
- `docs/tov-check.md` — onderhoudsgids voor de check + bewust-behouden-lijst
- `docs/tone-of-voice-alvah-site-nl.md` — de norm zelf (onveranderd)

Nieuwe content die via deze hook loopt, voldoet automatisch aan de
zero-tolerance-categorieën. Info-categorieën (verboden-woord, uitroepteken,
vuller, aandoening-eerst, ai-cliche) blijven zichtbaar in de output zodat
een schrijver ze bewust kan wegen.
...
| 11 | klaar | Tone-of-voice review alle content | 0 em-streepjes, pre-commit hook actief, onderhoudsgids in `docs/tov-check.md` |
