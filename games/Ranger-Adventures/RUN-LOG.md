...
---
**2026-07-12 03:55:52Z** · ticked: - [x] GATE-D2 · Fable re-judge — is he Alvah? Capture; judge the child scale
...
---
**2026-07-12 04:16:06Z** · ticked: - [x] D3.15 · Capture scenes run crash-free: the controls-hud + game3d-corsi frames land again [laptop] (poort-append 2026-07-12, poort-audit — the fresh 04:46 capture carries exactly TWO GAP entries and both are scene-code failures, not budget kills: `controls-hud` died on `ReferenceError: tx is not defined` inside the D1.5 pinned-spot walk code (`app/e2e-capture/` — an undefined variable in the page.evaluate; the crash means the ONE frame that evidences D1.5's terrain-sightline law at its four-audit conviction spot is MISSING, so that law stays pixel-unproven at the pinned spot), and `game3d-corsi` timed out 15 s waiting for `.explore-board-open` (its board approach — NB the main `board-affordance` scene landed near=true at 1.6 m this capture, so the confirmed-near walkTo works; corsi's entry path needs the same treatment or a longer honest wait). Harness-only, deliberately NARROW: fix the `tx` ReferenceError so the pinned-spot walk runs and the controls-hud frame + its terrain-inclusive `viewClear` samples land, and make game3d-corsi's board-open approach converge the way board-affordance now does. EXPLICITLY NOT this box: the parked D1.6 scope (heli-enter spec convergence — it failed its 5th capture and stays parked for Floris's hands-on batch). Verify by: fresh annotations hold ZERO GAP entries with `controls-hud` + `game3d-corsi` present as real frames (per-frame drawCalls <150), and the controls-hud annotation's `viewClear`/`terrainLift` match the pixels (a readable frame, or an honest lift) — that frame is the next poort-audit's re-court for the D1.5 terrain leg.
...
---
**2026-07-12 05:47:38Z** · ticked: - [x] D3.16 · The follow camera keeps the ranger readable at the jeep approach — no void frame, no lying viewClear [both] (poort-append 2026-07-12, poort-audit — fresh `18-jeep-near` is a BREACH frame on the real player path: at the jeep approach the follow camera collapses to dist 1.93 / pitch −1.17 (near straight-down, way outside the −0.33…−0.58 follow band) 0.7 m from the ranger, a large speckled BIRD model (the nachtzwaluw family) fills the near foreground swallowing him, the jeep's green marker ring reads as a giant hoop filling the frame, and the hook holds `viewClear`=true + `canopyFade`=1 while `avatarScreen.visible` is honestly FALSE — the D1.5 sightline test covers crowns + terrain but NOT animal/prop/vehicle meshes, so it lies for every other occluder class). Fix in game code, same comfort-safe family as D1.5: (1) the cam→avatar clear-line test covers EVERY rendered occluder class (props, animals, vehicles — not only crowns and terrain), fading faders and honestly reporting non-faders, so `viewClear` can never claim clear while the ranger is swallowed; (2) keep the vehicle-approach follow camera inside the normal pitch band (the D1.3 above-terrain + D1.5 boom machinery is the precedent — damped, never player-initiated-feeling, RM = cut); (3) the nearby-marker ring must not swallow the frame at close range (cap its screen-projected size or fade it out as the camera nears — it is wayfinding UI, doc §3, not a set piece). Verify by: shot (fresh `jeep-near`: ranger + jeep + ground context readable, no void, no frame-filling ring) + drive-assert (across the jeep approach `viewClear` matches `avatarScreen.visible` every sampled frame — zero clear-while-swallowed samples) + assert (pitch inside the follow band at the jeep-near snap; drawCalls <150; RM freeze pairs unaffected).
...
---
**2026-07-12 06:18:38Z** · ticked: - [x] D3.17 · Composed screens never show the low-LOD stand-in world [both] (poort-append 2026-07-12, poort-audit — fresh `d17-titlereturn-17` catches the TITLE mid-roundtrip rendering LOD cone-pines + a faceted low-poly oak + bare lawn with NO cabin and NO hero trees, while the main-flow `12-title-return` shows the full rich scene: under load (capture-throttled page ≈1 min after boot; a slow device or first-boot stream lags the same way) the title reframe reveals before the hero GLBs/LOD promotion are ready — a §2.3 one-fidelity break on the game's MOST-SEEN composed screen, beside §8.7's pre-settle family). Fix in game code: the title (and any composed reframe: board, mission entry) waits for/forces full-fidelity assets near its composed camera before revealing — hold the transition until the hero props in the title frustum are loaded + LOD-promoted (a readiness gate, not a spinner; under RM the reveal is already a cut), or keep the vista's near ring always-resident. Verify by: shot (the d17 title-leg frame reads full-fidelity: cabin + hero trees present, zero cone stand-ins in the title frustum — same bar for the main-flow title frames) + assert (drawCalls <150; title→world return still lands inside D1.7's settle window).
