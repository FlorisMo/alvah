...
# headless Claude sitting resuming from WORLD-LEDGER.md. The run works on the
# `run2-world` branch — deploys to alvah.nl happen ONLY from main, so Alvah's
# live game stays playable until Floris merges (or the run merges at the
# named milestones, WORLD-PLAN §6). Loop ends when the ledger has no [ ]
# left, the safety cap is hit, or progress stalls.
#
# Usage (from anywhere):
...
#   BRANCH=main bash .../world-run-loop.sh                  # opt out of branch isolation
#
...
BRANCH="${BRANCH:-run2-world}"
stall=0

# Work on the run branch; main stays the deployed game until a merge milestone.
if [ "$BRANCH" != "$(git branch --show-current)" ]; then
  git switch "$BRANCH" 2>/dev/null || git switch -c "$BRANCH" || { echo "✗ cannot switch to $BRANCH"; exit 1; }
fi
git push -u origin "$BRANCH" >> "$LOG" 2>&1 || echo "  ⚠ initial push of $BRANCH failed (offline?) — continuing locally" | tee -a "$LOG"

# First unchecked ledger line's text (empty when none remain).
next_box() { grep -m1 -E '^[[:space:]]*-[[:space:]]*\[ \]' "$LEDGER" 2>/dev/null; }

PROMPT='Resume the autonomous "Ranger van de Veluwe" WORLD run (run 2: one immersive 3D open world). You work in the alvah repo on branch run2-world; the game lives in games/Ranger-Adventures/ (outside src/).
...
7) Commit+push at each phase boundary and after the demo-critical boxes W0.4, W1.2, W1.5, W2.1: RUN_LEDGER=WORLD-LEDGER.md node games/Ranger-Adventures/app/scripts/ranger-run.mjs commit "<message>". This pushes the run2-world branch — deploys happen only from main. Merge run2-world into main ONLY when a ledger box says so (end of W2 with everything green, and W7.4). Never commit .env.local, assets-gen/, node_modules, playwright-report, test-results or e2e/__shots__ (W0.1 adds them to app/.gitignore).
8) If a box is blocked on Floris, surface it: RUN_LEDGER=WORLD-LEDGER.md node games/Ranger-Adventures/app/scripts/ranger-run.mjs status --blocker="<what you need>" (clear later with --blocker=none). Tick the blocked box ONLY if the plan marks it graceful-degrade (W0.7, W3.0, W3.5); otherwise leave it open, work the NEXT box, and say so in the log.
...
echo "=== WORLD run loop started $(date '+%F %T') · cap ${MAX_RUNS} · model ${MODEL:-default} · branch ${BRANCH} ===" | tee -a "$LOG"
