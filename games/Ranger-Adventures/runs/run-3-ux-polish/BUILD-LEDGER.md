...
- [x] P3.3 · **ONE hint system: F-06 + F-15 + F-11-hint + F-13 tracker move [both] +demo.** Sequence onboarding one hint at a time; ≤7-word lines; control hint persists until first successful use then fades; the 16-word toast becomes short transient(s); laptop help chip (≥56 px) re-shows hints; relocate the iPad tracker out of the joystick quadrant. One-time-hint flags via `state.ts` in the `alvah-ef-v1` ranger namespace (no new keys). **Verify by:** shot (single hint at entry, ≤7 words; iPad tracker not under the stick) + assert (hint-state field; joystick ∩ tracker boundingBox = ∅ on ipad). **Read-aloud firing: needs Floris demo.** — implemented, awaiting Floris demo (NOT fixed)
- [x] GATE-P3 · **Fable re-judge — Phase 3 (chips, navigation, hints — both platforms).** Re-capture; re-judge the token sizes, the back-paths and the hint sequencing. May re-open. Read-aloud stays demo-gated.
