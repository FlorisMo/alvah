# RUN-C-DIRECTION — the art-direction bible for Run C

> **Authored by the Fable art director, 2026-07-04 (P0.1 — the run's first act,
> VISION §10).** This is the single source of truth every Run C sitting
> re-reads and converges to. It operationalizes the locked VISION
...
- **Cloud shadows** must stay soft and feathered with ground detail readable
  inside them (Run B carry-over P1.8, §9).
...
9. **Pause + settings + avatar panels.** One overlay language (§3 intro);
   ...
   law (carry-over P1.9). The avatar creator inherits the same warm panel and
   golden backdrop.
...
## 9. Run B carry-over — triage record (Floris, 2026-07-04)

`runs/run-3-ux-polish/DEFERRED.md` holds **4 parked boxes** (each failed its
own screenshot grade 3 sittings running). Honest note on "the avatar-scale /
follow-camera / spawn-fade nut": Run B DID crack the scale core late in the
run (avatar reads at 1.7 m — HANDOFF-MONITOR.md; pose fields
`avatar.height`/`cam.dist` are in the annotations). What survived to
DEFERRED is the **camera-framing piece (F-19)** plus three siblings. All four
are now first-class Run C retry boxes, filed in RUN-C-LEDGER.md Phase 1 —
the framing/proportion one FIRST (P1.0, physically before the look boxes),
because the ranger and hero subjects must read at believable proportion in
frame before any screen can be judged "beautiful":

- **P1.0** ← Run B P1.6 / F-19: frame the board face on approach
  (damp-turn follow cam, board-in-frustum hook) + re-prove the proportion
  baseline in the same frame.
- **P1.7** ← Run B P1.7 / F-24 residue: the frisling must read as a young
  boar and "het gras" as grass (also §5 rank 4 covers the model side).
- **P1.8** ← Run B P1.8: soften/feather the drifting cloud-shadow layer.
- **P1.9** ← Run B P3.2 residue: the Instellingen panel's exit on-screen
  without scrolling (sticky ≥56 px `.tw-back`, no cut-off toggle row) — the
  last off-fold trap of Run B's navigation law.
