...
- [ ] World build-out: THREE.Terrain biomes (heide/bos/stuifzand/ven), instanced vegetation/props, character controller (three-mesh-bvh), wayfinding + diegetic HUD (5)
  ...
  - [ ] 38c · Character controller — kinematic capsule ground-follow + soft collision so the ranger can't walk through pines / into the ven / out of bounds (three-mesh-bvh or analytic clamp), tap-to-walk preserved, §1e camera untouched (1)
  - [ ] 38d · Wayfinding + diegetic HUD — in-world marker labels + a calm direction/distance cue to the active mission (no minimap chrome), veldnotitie-style diegetic HUD; ≥56px, dual-channel, reduced-motion-safe (1)
